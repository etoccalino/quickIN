...
class Trello:
    ...
    def _retry_after_renewing_token(self, network_call_to_trello, *args, **kwargs):
        ...
        try:
            ...
            if http.response is not None and http.response.status_code == 401:
                LOG.warning('Got a 401 response. Renewing the auth token...')
                self._refresh_token()
                LOG.info('Token renewed.')
                retry_network_call = True
        else:
            LOG.error('Got a "requests" error. Either a connection, network, or auth error')
            LOG.error(http)
            LOG.info('Review the config file for errors (e.g.: key, token)')
            raise
        ...
        if retry_network_call:
            ...
            try:
                ...
                LOG.error('Another error. Bailing out...')
                raise
    ...
    def post_new_card(self, title, description=None):
        ...
        LOG.info('Sending new card to Trello')
    ...
    def _prompt_user_for_token(self):
        ...
        url = self._trello.get_token_url(self._config['authentication']['app_name'],
                                         ...
                                         write_access=True)
        LOG.info(f'Will now attempt to renew the authorization token for "{app}".')
        LOG.debug(f'Opening a browser to {url}')
        webbrowser.open(url)
        token = input(f'Please authorize "{app}" and copy-paste the token here: ')
        LOG.debug(f'got {token=}')
        return token
    ...
    def _find_list_id(self):
        ...
        if list_id := self._config.get('trello', {}).get('list_id'):
            LOG.debug(f'found {list_id=} in the config')
            return list_id
        ...
        LOG.info('Fetching the target list by inspecting Trello.')
        LOG.debug(f'searching for {list_name=} in board with ID={board_id}')
        ...
        LOG.debug(f'searching for {list_name=} in lists: {lists}')
        try:
            list_id = [l['id'] for l in lists if l['name'] == 'IN'][0]
        except KeyError:
            LOG.error('Unrecognized format of list data from Trello!')
            LOG.warning('This error most likely requires patching the program code.')
            raise
        except IndexError:
            LOG.error(f'Could not find list with name "{list_name}"!')
            LOG.warning('Either the Trello workspace or the config is in an invalid state')
            LOG.info(f'Verify the Trello with ID={board_id} has a list with name={list_name}')
            raise

        # Update the config
        LOG.debug(f'Got {list_id=}. Persisting to the config.')
        ...
        while True:
            match window.read():
                ...
                case ('in_Return', values) if values.get('in') is not None:
                    ...
                    window['in'].set_focus()
                case event, values:
                    LOG.warning(f'Unrecognized GUI event: {event=}. Ignoring...')
                    LOG.debug(f'{event=} and {values=}')
                    window['in'].set_focus()
